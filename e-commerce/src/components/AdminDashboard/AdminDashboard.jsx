...
export function AdminDashboard() {
  ...
  return (
    <div className="flex flex-row flex-wrap">
      {/* Admin Dashboard */}
      <aside className={`w-64 min-w-2 overflow-y-auto border-r bg-slate-200 px-5 py-8 ${isMenuOpen ? 'block' : 'hidden'} lg:block`}>
        <div className="mt-6">
      ...
      <div className="lg:hidden">
        {isMenuOpen ? (
          <PanelRightIcon
            onClick={toggleMenu}
            className="text-black h-6 w-6 cursor-pointer absolute top-4 right-8 z-50"
          />
        ) : (
          <PanelLeftIcon
            onClick={toggleMenu}
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 bg-gray-700 bg-opacity-75 z-50">
          <div className="absolute inset-y-0 left-0 w-64 bg-slate-200">
            <div className="p-4">
              ...
              <nav className="space-y-3">
                <NavLink
                  className="block text-gray-950 hover:text-white hover:bg-slate-700"
                  to="/add"
                  onClick={closeMenu}
                >
                  Add Product
